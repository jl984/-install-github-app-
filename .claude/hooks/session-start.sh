...
# Option 1: OAuth token saved by authenticate.py (preferred)
if [ -f "$TOKEN_FILE" ]; then
  echo "export NOTEBOOKLM_TOKEN_FILE=$TOKEN_FILE" >> "$CLAUDE_ENV_FILE"
  echo "OAuth token loaded from $TOKEN_FILE"

# Option 2: Service account JSON passed as a secret
...
# Option 3: Raw session cookie passed as a secret
  ...
  echo "Run 'python authenticate.py' to log in via browser, or set"
  echo "GOOGLE_CREDENTIALS_JSON / NOTEBOOKLM_COOKIE in your project secrets."
fi
